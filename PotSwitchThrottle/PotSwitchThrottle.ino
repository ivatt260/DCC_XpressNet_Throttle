...
3) This file contains lots of prints for possible XPressNet messages. These
are here but (so far) are not called. They will eventually be removed, once
the software is shown to be stable.

4) It expects headlight is at DCC F0; while dimming not currently supported. Headlight
is turned on/off automatically when movement is detected. See the constants below
...
Digital Pin 13 - LED - solid on for ok; flashing for "track power off"
           (see more possible messages below)
...
Pin 1  RO Serial rx   -   Arduino Serial receive
Pin 2  /RE rec ena     -  Arduino Pin 12
Pin 3  DE send ena     -  Arduino Pin 12
Pin 4  DI serial tx    -  Arduino Serial transmit
Pin 5  GND             -  same as the Arduino ground
Pin 6  A               -  XPressNet connector "A"
Pin 7  B               -  XPressEnt connector "B"
Pin 8  VCC             -  supplied from the Arduino.
...
7) 2 inputs  as outlined in the Arduino connections above; one is a SPDT (centre
   ...
   contact will be the "ground" connection.
   
   a Linear taper potentiometer; really any value from 10K above should work fine,
   one side tied to ground, the other to vcc (5v) and the wiper to analogue pin A0.
...
8) We communicate at 62.5k, 1 start bit, 9 data bits, 1 stop bit, no parity. Note that
   ...
   Without the serial receive, nothing happens. 
...
void setup() {
 ...
 dataSaved = 0;
  ...
  #ifdef VERBOSE
  Serial.print ("myAddress is  "); Serial.print (MY_ADDRESS); Serial.print ("  "); Serial.println (MY_ADDRESS,HEX);
  Serial.print ("myDirectedOps "); Serial.println (myDirectedOps,HEX);
  Serial.print ("myDirectedWin "); Serial.println (,HEX);
  #endif //VERBOSE
